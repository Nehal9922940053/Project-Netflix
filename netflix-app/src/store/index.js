...
export const getGenres = createAsyncThunk("netflix/genres", async () => {
    const {
        ...
        "https://api.themoviedb.org/3/genre/movie/list?api_key=e01ec715d1c78e4a0db1070f085581f9"
        );
    return genres;
});
...
async({genre,type}, thunkAPI)=>{
    const {
      ...
      return getRawData(
        `https://api.themoviedb.org/3/discover/${type}?api_key=e01ec715d1c78e4a0db1070f085581f9&with_genres=${genre}`,
        genres
        );
        // console.log(data);
    }
...
export const removeFromLikedMovies =createAsyncThunk("netflix/deleteLiked",async({email,movieId})=>{
    const{ data: {movies},
} = await axios.put(`http://localhost:8000/api/user/delete`,{
        email,
        movieId,
    });
    return movies;
